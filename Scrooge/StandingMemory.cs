...
/// </summary>
internal static class StandingMemory
{
  /// <summary>
  ...
  /// the self-heal round closes it the moment its item is processed, and the
  ...
  /// <summary>
  /// Flag classes the PINCH PASS DOES NOT OWN - raised elsewhere (the Ledger's
  /// standing-lane scorer), against evidence a pricing pass never looks at.
  ///
  /// <para>Self-heal's contract is "this pass re-evaluated the rule and it did
  /// not fire, so the condition resolved". A pass that never asks the question
  /// has not answered it, and closing on that silence would delete a live flag
  /// the moment its item was pinched - which is every flag of this class, within
  /// one run of being raised.</para>
  ///
  /// <para>The exemption is only half a contract, and the other half is not
  /// optional: THE RAISER MUST CLOSE ITS OWN, or this list is just a licence to
  /// mint immortal flags (see V36, and the migration it cost to bury them). For
  /// melt_beats_ask that half lives in <see cref="BoardListings.MeltContestsToClose"/>,
  /// run over the whole standing book every time the ledger refreshes. Nothing
  /// belongs on this list until its closer exists.</para>
  /// </summary>
  internal static readonly IReadOnlySet<string> ForeignReasons =
    new HashSet<string>(StringComparer.Ordinal) { BoardListings.MeltBeatsAskReason };
  ...
  ///
  /// <para>Except <see cref="ForeignReasons"/>: silence is only evidence of
  /// resolution when the pass actually asked the question.</para>
  /// </summary>
    ...
    IReadOnlySet<string> raisedThisPass)
  {
    ...
    foreach (var (id, reason) in openFlags)
      if (!raisedThisPass.Contains(reason) && !ForeignReasons.Contains(reason))
        toClose.Add(id);
    return toClose;
  }
  ...
  /// <summary>What UpsertStandingFlag should do with an evidence-keyed flag.</summary>
  ...
  /// touching a price).
  /// </summary>
  internal static bool ItemSkipped(StandingAction queuedAction, PricingResult result)
    => queuedAction == StandingAction.None || result == PricingResult.Skipped;
  ...
  /// <summary>An open lane_held flag as the zombie round sees it.</summary>
  ...
  /// <summary>
  /// The batch zombie round (pure/tested; the storage layer reads rows and stamps the
