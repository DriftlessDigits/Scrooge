...
/// the 07-22 "rows ride along forever" bug dies without a sixth round stage.
///
...
/// lives in the pinch's task chain (VendorRiderExecutor). Kept Dalamud-free and linked into
...
/// <see cref="BoardConfidence.BulkSet{T}(IEnumerable{ValueTuple{T, ConfidenceTier}})"/>,
...
internal static class VendorRider
{
  ...
  internal static List<T> Riders<T>(IEnumerable<(T Item, ConfidenceTier Tier)> candidates)
    => BoardConfidence.BulkSet(candidates);

  /// <summary>
  /// The rider WITH the player's own rulings threaded through - the same widening
  /// every other confirm got (Drift, 07-25: "if I made a decision on a pile, the pile
  /// specific button should use that"). A row a human explicitly staged to Vendor
  /// rides his next pinch regardless of tier; the click IS the resolution the
  /// confidence gate was waiting for. Without this the front-load gate would lie:
  /// it presents Pull &amp; Vendor rows as rulings that change this run, and a
  /// ruling the rider ignores changes nothing.
  /// </summary>
  internal static List<T> Riders<T>(
    IEnumerable<(T Item, ConfidenceTier Tier, bool PlayerResolved)> candidates)
    => BoardConfidence.BulkSet(candidates);
  ...
  /// <summary>Player-ruled riders, grouped by the retainer whose visit executes them.</summary>
  internal static Dictionary<string, List<T>> ByRetainer<T>(
    IEnumerable<(T Item, ConfidenceTier Tier, bool PlayerResolved)> candidates, Func<T, string> retainerOf)
  {
    var map = new Dictionary<string, List<T>>(StringComparer.Ordinal);
    foreach (var item in Riders(candidates))
    {
