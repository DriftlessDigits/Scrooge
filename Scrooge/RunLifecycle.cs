...
/// <summary>
/// The run-host lifecycle core (LanePricing/StandingMemory mold: no game reads, no
...
internal sealed class RunLifecycle
{
  ...
  private DateTime _lastItemAt; // last doneDelta>0 beat - the ETA's countdown anchor
  ...
  internal bool RecordProgress(int doneDelta, long valueDelta, DateTime now)
  {
    ...
    {
      Done += doneDelta;
      _lastItemAt = now; // countdown anchor: only ITEM beats recalibrate the ETA
    }
    Value += valueDelta;
    _lastProgressAt = now;
    return true;
  ...
  /// <summary>
  /// Self-calibrating COUNTDOWN ETA for the remaining items, or null when there is
  /// no honest basis for one (nothing left, or nothing observed yet and no seed).
  ///
  /// <para>The pace is frozen at item-completion boundaries (elapsed-at-last-item /
  /// done), never read off the live clock: a per-frame pace (elapsed / done) bills
  /// the wait on the CURRENT item to every future item, so the ETA climbs between
  /// completions and drops when one lands - the 2026-02-24 sawtooth, fixed then
  /// with a per-item countdown reset and reintroduced when the run-host lifecycle
  /// retired that hand-rolled ETA (693c4ef). This is that ruling, generalized:
  /// fresh estimate at each item, and between items the display ticks DOWN by
  /// wall clock (floored at zero - an overdue item reads "~0s left", honest).</para>
  ///
  /// Before the first item it counts down from the seeded per-item estimate if the
  ...
  internal TimeSpan? Eta(DateTime now)
  {
    ...
    double msPerItem;
    if (Done > 0)
      msPerItem = (_lastItemAt - _startedAt).TotalMilliseconds / Done; // pace frozen at the last completion
    ...
    var spentOnCurrent = (now - _lastItemAt).TotalMilliseconds;
    var etaMs = Math.Max(0, (msPerItem * remaining) - spentOnCurrent);
  ...
  // ===========================================================================
  // Hawk completion boundary - the M4 inventory-scope zombie round gate
  // ===========================================================================

  /// <summary>
  /// The gate for the Hawk run's M4 inventory-scope zombie round. A Hawk run may
  ...
  /// returns null and rounds nothing. Fail toward open flags, never toward a silent
  /// close. Pure sibling of the pinch board round wired in SnapshotListings.
  /// </summary>
  internal static IReadOnlySet<uint>? HawkRoundInputs(
