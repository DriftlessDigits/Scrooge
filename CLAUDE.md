...
- **Versioning** (sharpened 2026-09-01): minor = new capability the player can see; patch = fixes and refinements, visible or not (a fix being visible does not make it a feature - 3.0.2.0 shipped hidden preview rows and a chat line as a patch). Pick the slot honestly.
...
1. **Pure decision core** (Dalamud-free, test-linked): `LanePricing` (the pricing spine: listings judged as crashers / competition / dreamers against a recency-weighted sale lane - "A11" in doc comments; regime segments, bands), `RoutingRules` (four-exit router; factory defaults live ONCE in `RoutingDefaults`, read by both Configuration pegs and the RoutingConfig snapshot), `DesynthSkillup` (the SimpleTweaks-mirror color cascade), `TriageCase` (case voice + verdicts), `Spine`/`FlowPlan`/`GatePlan`/`BellPlan` (round planning), `ReceiptGrading`, `Durations` (four duration grammars - Span/Elapsed/Ago/DayAge - deliberately not unified; a pin test records their disagreements), `PriceFloor`, `PricingVoice`, `PaceBank`, the `*Schema` classes.
...
4. **Windows** (paint + press-forwarding only): `AccountantWindow` (shell + surface partials incl. `ScoreCell`), `GilWindow` (shell + per-tab partials), `ConfigWindow` (+ `ConfigWidgets` - incl. `KeyCombo`, the curated hotkey dropdown: modifiers/A-Z/0-9/F-keys, an off-list saved binding prepended never eaten), `LedgerWindow` (the run-log transcript - unrelated to `Board/LedgerCache`), `DesynthPreviewWindow`. `HawkWindow` was DELETED 2026-08-29 (probation closed) - the hawk RUN, `ListableInventoryScanner`, and sell-view navigation all stand; only the manual view and its Select-for-Sale gesture went. Shared grammar: `TableSort`, `TabCache`, `Format`, `ScroogeColors`, `StageRail`. Config tabs were fully reconciled against the knob registry 2026-08-23 (all ten; the Pricing tab is the style bar - gold `SectionHeader`, one plain subheader line, ALL mechanics behind `(?)` hovers); the tab once called Ledger is **Rounds** (`DrawRoundsTab`) and Hawk Settings is **Item Rules**.
...
- **The deep-cut guard is GONE** (delisted 2026-08-23, species removed in the 3.1 sweep): `MaxUndercutPercentage`, `UndercutTooDeep`, and the whole ConfirmedPrice confirm flow are deleted. The lane owns crasher defense.
...
- **Item rules are bag right-clicks, no window required** (2026-08-23): Always Vendor / Ban / their removals appear on any inventory context menu. Select for Sale died with the manual Hawk view (2026-08-29) - selection was meaningless without the surface it selected on.
- **The skillup color law** (2026-08-28, mirrors SimpleTweaks exactly): `DesynthSkillup.Classify(float skill, itemLevel, maxLevel)` - Green the moment skill reaches the ladder's top (`GameSafe.MaxDesynthLevel()`, cached sheet scan of max LevelItem among Desynth>0; 0 = sheet unavailable = cap branch disabled, never all-Green), equality is RED (strictly greater leaves Red), and the compare is the RAW float skill (truncation would paint the whole [n, n+1) band wrong). Skill-up worth prices off the color (`SkillupWorthRed`/`Yellow` pegs); Green or colorless = zero skillup gil in melt scores. The sitrep's "desynth skill" section prints the ladder top + all 8 raw skills (the field verify). Field-proven 2026-08-29 vs the game's own salvage paint.
- **Non-equipment desynthables enter the contest** (ruled 2026-08-28: "fish are the main way to train desynth for CUL"; scope corrected same night to ALL non-equipment desynthables - furnishings included): the board scan, color gate, no-evidence door, and route tags treat any Desynth>0 item like gear (plain mats still sit out). But they melt ONLY through the skillup door - no measured-yield or prior-based melt case - and they never feed nor borrow the gear band priors (belt and braces across the pure/game seam). Pins in `FishRoutingTests`.
- **The outbid clause names the SEATED List witness** (2026-08-29): a skillup melt's "Outbids..." sentence quotes the operand List actually carried - own sale, settled tape, or Look, whichever the ladder seated (the same number the board cell shows); when the seat is not the Look, the ask rides as a labeled aside ("Listed, you'd ask N"). Never quotes an unseated witness, never fires when the outbid isn't numerically true.
- **The melt walk** (2026-08-30/31, Phase B): melt is BAGS-ONLY and bag items live under exactly two salvage categories (Equipment=0, Housing=1 of `SalvageItemCategory`; `MeltWalk` pins the ordinals). The preview scans the INVENTORY side too (`DesynthInventoryScanner.ScanHiddenPile`) so pile items hidden behind the window's filter render as real rows tagged "hidden"; **checkboxes are the contract** - the run melts exactly the checked set, hidden checked rows are held aside and fed to a walk leg that flips `AgentSalvage.SelectedCategory` + `ItemListRefresh`, resolves each by `FindAgentIndex`, and restores the player's category on BOTH exits. Zero hidden checked = no flip. `MeltUnreachable` now fires only for pile items in NO bag (retainer stock). Field-proven 08-30 and 09-01.
- **Coffer pulls have two edges in either order** (2026-09-01): `CofferPullWatcher` pairs the bag-count drop and the "You obtain" line through `CofferPullMatch` (pure), banking on whichever arrives second - the chat line beat the poll by 13ms on a live box and the old arm-then-listen shape lost it. `coffer_pulls.kept_at` (V49) stamps the one fate the machine cannot see (the player learned the minion); SOLD and HELD stay derivable. Nothing reads the table until 3.1.
- **Seat is PRICE RANK** (ruled 2026-09-01): `SeatOf` counts strictly-cheaper rows; a matched ask is still seat 1. The seat exists to say when someone is cheaper than us, not to model tie order.
- **Desynth totals count ACTS, not slots** (2026-08-28): lifecycle ETA, ledger counts, `desynth_runs.total_items`, and the preview button all sum stack quantities; the preview says both when they differ ("2 selected — 7 desynths").
...
- **Gil Tracking is ALWAYS-ON** (3.1 sweep, the ruled posture: "you installed Scrooge, you get Scrooge"): `EnableGilTracking` and its ~30 guard sites are gone; receipts, true-ups, and the board freshness gate's clock (`RepinchFloorHours` reading the last completed full pinch's stamp from `GilTracker.FinalizeRun`) all run unconditionally.
- **The board read has ONE chain** (`BoardReadLadder`): every price-deciding run door - pinch, recon, hawk posting, and (since 2026-08-29) the standing reprice - enqueues the same ladder: whole-board completion against the proxy total, escalating retry windows, honest censored logging, never-answered = HOLD with a flag. The ONE remaining flat-wait door is the PostPinch hotkey re-read, runless by design (named in the ladder header; moving it is a design walk). `MarketBoardKeepOpenMS` is a const now, not a knob.
