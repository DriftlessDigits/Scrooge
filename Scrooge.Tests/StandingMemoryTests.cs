using Xunit;
using static Scrooge.StandingMemory;
...
/// </summary>
public class StandingMemoryTests
{
  ...
  public void SelfHeal_ClosesLegacyDeadProducerFlags()
  {
    ...
    // nothing can re-raise them, so processing their item always rounds them,
  ...
  [Fact]
  public void SelfHeal_NeverClosesAFlagThisPassNeverAsked()
  {
    // melt_beats_ask is raised by the Ledger's standing-lane scorer, not by a
    // pricing pass - so a pinch's silence about it is not a resolution. Without
    // the exemption every one of these flags would die the first time its item
    // was pinched, which is minutes after it was raised.
    var close = FlagsToClose(Flags("melt_beats_ask", "lane_held"), Raised(/* nothing fired */));
    Assert.Equal(new long[] { 2 }, close);
    Assert.Contains("melt_beats_ask", ForeignReasons);
  }
  ...
  // self-heal round never touches it. The zombie round closes it - but ONLY when
  ...
  public void ItemSkipped_RepriceWithStampedAction_IsNotSkipped()
  {
    ...
    Assert.False(StandingMemory.ItemSkipped(StandingAction.Reprice, PricingResult.Pending));
  }
  ...
  public void ItemSkipped_NoQueuedAction_IsSkipped()
  {
    ...
    Assert.True(StandingMemory.ItemSkipped(StandingAction.None, PricingResult.Pending));
  }
  ...
  public void ItemSkipped_SoldSinceFlagging_IsSkipped()
  {
    ...
    Assert.True(StandingMemory.ItemSkipped(StandingAction.Reprice, PricingResult.Skipped));
    Assert.True(StandingMemory.ItemSkipped(StandingAction.Vendor, PricingResult.Skipped));
  }

  [Fact]
  public void ItemSkipped_VendorAndPull_AreNotSkipped()
  {
    Assert.False(StandingMemory.ItemSkipped(StandingAction.Vendor, PricingResult.Pending));
    Assert.False(StandingMemory.ItemSkipped(StandingAction.Pull, PricingResult.Pending));
  }
}
