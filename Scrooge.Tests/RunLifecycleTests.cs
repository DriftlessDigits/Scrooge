...
public class RunLifecycleTests
{
  ...
  /// <summary>
  /// "IF A RUN BAILED WITHOUT PROCESSING AN ITEM, THEN IT BAILED" (review ruling S18,
  /// 2026-08-12). Recon and the bell both enqueue their run-end task behind the
  /// close-up steps, so a death that lands while those run leaves the straggler in the
  /// queue - and it went on to report the pass DONE over a run the round had already
  /// halted. The fix reads THIS return value rather than adding a flag beside the
  /// state: the refusal below is already the latch, and a second spelling of "did it
  /// end" is a second thing that can disagree.
  /// </summary>
  [Fact]
  public void TheRefusedTransitionIsTheLatch_AStragglerCannotReportDoneAfterDied()
  {
    var died = Started();
    Assert.True(died.Cancel(At(5)));            // the abort / draw catch / stall teardown
    Assert.False(died.Complete(At(6)));         // the straggler asks, and is told no
    Assert.Equal(Scrooge.RunState.Cancelled, died.State);

    // The natural end still gets its yes when nothing killed the run first, which is
    // what keeps the latch from being a silence.
    var finished = Started();
    Assert.True(finished.Complete(At(6)));

    // And the bell's all-retainers-full exit, which reports a DEATH: a second death
    // report over a run already dead would overwrite the round's halt with the wrong
    // gap.
    var alreadyDead = Started();
    alreadyDead.Cancel(At(5));
    Assert.False(alreadyDead.Cancel(At(6)));
  }
  ...
  [Fact]
  public void Eta_UsesSeed_BeforeFirstItem_AndCountsDown()
  {
    var run = new Scrooge.RunLifecycle();
    run.Start(10, Scrooge.RunValueUnit.Gil, T0, seededMsPerItem: 1000);
    // 0 done, 10 remaining, 1000ms/item seed = 10s budget; 3s already spent
    // waiting on the first item -> the countdown reads 7s, not a frozen 10.
    var eta = run.Eta(At(3));
    Assert.NotNull(eta);
    Assert.Equal(7_000, eta!.Value.TotalMilliseconds, 3);
  }
  ...
  [Fact]
  public void Eta_NeverClimbs_BetweenItemCompletions()
  {
    // THE 2026-02-24 SAWTOOTH, regression-pinned: a per-frame pace (elapsed/done)
    // bills the wait on the current item to every future item, so the ETA climbed
    // between completions. Ruled fix: pace freezes at item boundaries and the
    // display ticks DOWN by wall clock between them.
    var run = Started(total: 10);
    run.RecordProgress(4, 0, At(8)); // 2s/item frozen; 6 remaining = 12s budget
    var atCompletion = run.Eta(At(8))!.Value;
    var oneSecondLater = run.Eta(At(9))!.Value;
    var fiveSecondsLater = run.Eta(At(13))!.Value;
    Assert.Equal(12_000, atCompletion.TotalMilliseconds, 3);
    Assert.Equal(11_000, oneSecondLater.TotalMilliseconds, 3); // ticks down
    Assert.Equal(7_000, fiveSecondsLater.TotalMilliseconds, 3); // keeps ticking down
  }

  [Fact]
  public void Eta_FloorsAtZero_WhenCurrentItemRunsOverdue()
  {
    // 1s/item pace, 1 remaining, but the current item has taken 30s: the countdown
    // reads ~0s left (honest "any moment now"), never a negative span.
    var run = Started(total: 10);
    run.RecordProgress(9, 0, At(9));
    var eta = run.Eta(At(39));
    Assert.NotNull(eta);
    Assert.Equal(0, eta!.Value.TotalMilliseconds, 3);
  }

  [Fact]
  public void Eta_ValueOnlyBeats_DoNotMoveTheCountdownAnchor()
  {
    // A value beat (gil landing, no item finished) resets the stall watchdog but
    // must NOT recalibrate the ETA - only item completions carry pace information.
    var run = Started(total: 10);
    run.RecordProgress(4, 0, At(8));      // 2s/item, 6 remaining = 12s budget
    run.RecordProgress(0, 500, At(10));   // value-only beat, 2s into the current item
    var eta = run.Eta(At(10));
    Assert.Equal(10_000, eta!.Value.TotalMilliseconds, 3); // 12s - 2s spent, anchor unmoved
  }
  ...
  [Fact]
  public void HawkRoundInputs_FullObservation_ReturnsObservedSet()
  {
    IReadOnlySet<uint> observed = new HashSet<uint> { 100u, 200u, 300u };
    var inputs = Scrooge.RunLifecycle.HawkRoundInputs(fullyObserved: true, observed);
    Assert.NotNull(inputs);
    Assert.Equal(observed, inputs);
  }

  [Fact]
  public void HawkRoundInputs_PartialObservation_ReturnsNull()
  {
    IReadOnlySet<uint> observed = new HashSet<uint> { 100u, 200u };
    // Aborted early / cancelled / stalled / incomplete walk -> no round, flags stay open.
    Assert.Null(Scrooge.RunLifecycle.HawkRoundInputs(fullyObserved: false, observed));
  }

  [Fact]
  public void HawkRoundInputs_FullButEmpty_StillReturnsSet()
  {
    ...
    var inputs = Scrooge.RunLifecycle.HawkRoundInputs(fullyObserved: true, empty);
    Assert.NotNull(inputs);
    Assert.Empty(inputs!);
  }
