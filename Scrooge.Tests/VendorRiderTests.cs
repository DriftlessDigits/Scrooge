...
public class VendorRiderTests
{
  ...
  // ---- The player's own rulings (WALK unit 5: the gate must not lie) ----

  private static (Row, ConfidenceTier, bool) Ruled(uint id, string retainer, ConfidenceTier tier, bool ruled)
    => (new Row(id, retainer), tier, ruled);

  [Fact]
  public void Riders_APlayerRuledRowRidesRegardlessOfTier()
  {
    // The front-load gate presents Pull & Vendor rows as rulings that change this
    // run. A ruling the rider ignored would make the gate a liar - so a hand-staged
    // Vendor click rides exactly like a unanimous row.
    var candidates = new[]
    {
      Ruled(1, "Bender", ConfidenceTier.Unanimous, false),
      Ruled(2, "Bender", ConfidenceTier.Mixed, true),
      Ruled(3, "Bender", ConfidenceTier.Contradicted, true),
      Ruled(4, "Bender", ConfidenceTier.Mixed, false),   // still nobody's decision
    };

    Assert.Equal(new uint[] { 1, 2, 3 },
      VendorRider.Riders(candidates).Select(r => r.Id).OrderBy(x => x));
  }

  [Fact]
  public void Riders_UnruledMixedRowsStaySilent()
  {
    var candidates = new[]
    {
      Ruled(1, "Bender", ConfidenceTier.Mixed, false),
      Ruled(2, "Fry", ConfidenceTier.Contradicted, false),
    };
  ...
  [Fact]
  public void ByRetainer_GroupsPlayerRuledRidersToo()
  {
    var candidates = new[]
    {
      Ruled(1, "Bender", ConfidenceTier.Unanimous, false),
      Ruled(2, "Fry", ConfidenceTier.Contradicted, true),
      Ruled(3, "Fry", ConfidenceTier.Mixed, false),
    };

    var byRetainer = VendorRider.ByRetainer(candidates, r => r.Retainer);

    Assert.Equal(new uint[] { 1 }, byRetainer["Bender"].Select(r => r.Id));
    Assert.Equal(new uint[] { 2 }, byRetainer["Fry"].Select(r => r.Id));
  }
